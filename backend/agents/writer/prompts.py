...
- 必须遵守当前 StylePacket 的风格摘要、设计原则、样例素材和路径约束
- 如果 StylePacket 明确列出了 Path B 禁用词，`image_prompt` 绝对不能出现这些词
- 如果 StylePacket 只支持单一路径，所有页面的 `path_hint` 必须与该路径一致
- 不要写与当前风格无关的通用 image_prompt
...
7. 必须遵守上面的 StylePacket 约束和 Path B 禁用词
...
6. 必须遵守当前 StylePacket 的路径限制和 Path B 禁用词。
7. `title` 必须保留 outline 里的断言句标题，不能改写成主题词。
8. `text_to_render.title` 在 Path B 页面必须控制在 8 个中文字以内。
9. 所有 bullet 文案都必须控制在 12 字以内。
"""
