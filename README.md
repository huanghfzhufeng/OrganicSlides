...
- 🔐 **用户认证** - JWT 安全认证系统
- ⚙️ **服务拆分** - API 网关与 Worker 队列执行器分离运行
- ♻️ **任务恢复** - worker 中断后可回收 stale job，重复请求复用已完成结果
- 🗃️ **对象存储** - PPT、缩略图和渲染产物通过 object storage 管理
- 🐳 **Docker 部署** - 一键启动完整服务
...
└──────────┴──────────┴──────────┴──────────┴────────────────┘
 ...
 PostgreSQL (state/jobs/events) + Redis (optional cache) + Object Storage
```
...
- **对象存储**: Local backend（默认）或 MinIO/S3-compatible backend
- **认证**: JWT + bcrypt
...
# Worker 轮询任务队列的间隔（秒）
WORKER_QUEUE_POLL_INTERVAL_SECONDS=1.0

# 判定 worker 已中断并允许回收任务的超时（秒）
WORKER_JOB_STALE_SECONDS=120

# worker 运行中刷新任务活跃度的心跳间隔（秒）
WORKER_JOB_HEARTBEAT_SECONDS=15

# 对象存储（本地默认；docker-compose 会覆盖为 MinIO）
OBJECT_STORAGE_BACKEND=local
OBJECT_STORAGE_LOCAL_ROOT=object_storage
OBJECT_STORAGE_PUBLIC_BASE_URL=http://localhost:8000/api/v1/assets
OBJECT_STORAGE_BUCKET=masppt-assets
...
ASSET_CLEANUP_INTERVAL_SECONDS=300
ASSET_CLEANUP_BATCH_SIZE=100
```
...
│   ├── worker_runtime.py       # Worker 队列消费与工作流执行器
│   ├── job_queue.py            # 任务入队逻辑
│   ├── services/object_storage.py # 对象存储抽象
